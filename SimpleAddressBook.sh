...
addToRecord()
{
	...
	do
		...
		echo "If you'd like to quit, enter 'q'else press any other key to continue."
		read cont
		if [ "$cont" == 'q' ]
                        then
                        break
		...
		read zip
#		patLast="^[A-Z]{1}[a-z]{2,}$"
#		patFirst="^[A-Z]{1}[a-z]{2,}$"
#		patEmail="^[a-zA-Z0-9]+([._+-][a-zA-Z0-9]+)*@[a-zA-Z0-9]+[.]+[a-z]{2,4}([.][a-z]{2})*$"
#		patPhone="^[+91]{3}[ ][0-9]{10}$"
#		patCity="^[A-Z]{1}[a-z]{2,}$"
#		patState="^[A-Z]{1}[a-z]{2,}$"
#		patZip="^[0-9]{3}[ ]?[0-9]{3}$"
#		if [[ $patLast =~ $lastName && $patFirst =~ $firstName && $patEmail =~ $emailId && $patPhone =~ $phnNo && $patCity =~ $city && $patState =~ $state && $patZip =~ $zip ]]
#		then
#			echo "Valid Entries"
#		else
#			echo "Invalid Entries, Please check."
#			break
#		fi
